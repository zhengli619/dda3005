import numpy as np
from numpy.linalg import norm
from PIL import Image
from matplotlib import pyplot as plt
import os
from scipy.linalg import qr, svd, lu_factor, lu_solve, solve_triangular
import time



# Define directories
source_image_dir = r'C:\dda3005\project\source_images'
blurred_image_dir = r'C:\dda3005\project\blurred_images'
recovered_image_dir_LU = r'C:\dda3005\project\recovered_images_LU'
recovered_image_dir_QR = r'C:\dda3005\project\recovered_images_QR'
recovered_image_dir_householderQR = r'C:\dda3005\project\recovered_images_householderQR'
recovered_image_dir_householder_pivoting_QR = r'C:\dda3005\project\recovered_images_householder_pivoting_QR'
recovered_image_dir_scipy_pivoting_QR = r'C:\dda3005\project\recovered_images_scipy_pivoting_QR'
recovered_image_dir_QR_withRnonsingular = r'C:\dda3005\project\recovered_images_QR_withRnonsingular'

# Parameters for Aleft
j_Aleft = 1
k_Aleft = 5
# Parameters for Aright
j_Aright = 0
k_Aright = 20  # Meaning the motion looks like moving to the right

# Helper function to convert RGB/RGBA to grayscale
def convert_RGB_RGBA_to_grayscale(img):
    """
    Convert an RGB or RGBA image to grayscale.
    
    Parameters:
    - img: np array, image data
    
    Returns:
    - img_gray: np array, grayscale image
    """
    img = np.copy(img)
    if len(img.shape) == 3 and img.shape[2] in [3, 4]:
        if img.shape[2] == 4:
            img_rgb = img[:, :, :3]
        else:
            img_rgb = img
        img_gray = 0.2989 * img_rgb[:, :, 0] + 0.5870 * img_rgb[:, :, 1] + 0.1140 * img_rgb[:, :, 2]
        return img_gray
    else:
        return img  # Already grayscale

# Helper function to make matrix non-singular using SVD
def make_matrix_non_singular(A, epsilon=1e-6):
    """
    Make matrix A invertible by modifying its singular values.
    
    Parameters:
    - A: the input matrix (numpy.ndarray)
    - epsilon: threshold
    
    Returns:
    - A_non_singular: the output invertible matrix (numpy.ndarray)
    """
    
    U, S, VT = svd(A, full_matrices=False)

    # Modify singular values: if < epsilon, set to epsilon
    S_adjusted = np.where(S < epsilon, epsilon, S)

    # Construct new diagonal matrix S'
    Sigma_adjusted = np.diag(S_adjusted)

    # Construct A' = U * S' * V^T
    # print("Constructing A' (modified matrix)...")
    A_non_singular = U @ Sigma_adjusted @ VT
    # print("Construction of A' completed.")

    return A_non_singular

# Class to create motion type blurring matrices
class MotionTypeBlurringMatrix:
    def __init__(self, n, k, j=0):
        """
        Initialize the motion type blurring matrix.
        
        Parameters:
        - n: Dimension of the matrix (n x n)
        - k: Parameter k
        - j: Parameter j, default is 0
        """
        self.n = n
        self.k = k
        self.j = j
        self.blurring_matrix = np.zeros((n, n))
        self._initialize_blurring_matrix()

    def _initialize_blurring_matrix(self):
        """
        Initialize the blurring matrix based on motion parameters.
        """
        a_values = self._compute_a_values()
        # Fill the upper triangle
        for i in range(1, self.n + 1):
            for j_idx in range(1, i + 1):
                key = f'a_{i}'
                self.blurring_matrix[j_idx - 1, self.n - i + j_idx - 1] = a_values.get(key, 0)
        # Fill the lower triangle
        for i in range(self.n + 1, 2 * self.n):
            for j_idx in range(1, 2 * self.n + 1 - i):
                key = f'a_{i}'
                self.blurring_matrix[i - self.n + j_idx - 1, j_idx - 1] = a_values.get(key, 0)

    def _compute_a_values(self):
        """
        Compute the a_i values for the blurring matrix.

        Returns:
        - a_dict: Dictionary of a_i values
        """
        a_dict = {f'a_{i}': 0 for i in range(1, 2 * self.n)}
        for i in range(0, self.k):
            key = f'a_{self.n + self.j - i}'
            a_dict[key] = (2 / (self.k * (self.k + 1))) * (self.k - i)
        return a_dict

    def get_matrix(self):
        """
        Retrieve the blurring matrix.

        Returns:
        - blurring_matrix: np array, blurring matrix
        """
        return self.blurring_matrix

# Function to apply motion blur to an image
def apply_motion_blur(img, n, j_Aleft, k_Aleft, j_Aright, k_Aright):
    """
    Apply motion blur to an image using Aleft and Aright matrices.

    Parameters:
    - img: np array, grayscale image
    - n: int, size of the image (assuming square)
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix

    Returns:
    - B: np array, blurred image
    """
    motion_blurring_Aleft = MotionTypeBlurringMatrix(n, k_Aleft, j_Aleft)
    Aleft = motion_blurring_Aleft.get_matrix()

    motion_blurring_Aright = MotionTypeBlurringMatrix(n, k_Aright, j_Aright)
    Aright = motion_blurring_Aright.get_matrix()

    B = Aleft @ img @ Aright
    return B

# Function to blur all PNG files in the source directory
def blur_all_png_files(source_dir, j_Aleft, k_Aleft, j_Aright, k_Aright, blurred_dir):
    """
    Apply motion blur to all PNG images in the source directory.

    Parameters:
    - source_dir: str, path to source images
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix
    - blurred_dir: str, path to save blurred images
    """
    if not os.path.exists(blurred_dir):
        os.makedirs(blurred_dir)
        print(f"Created directory for blurred images: {blurred_dir}")

    for filename in os.listdir(source_dir):
        if filename.lower().endswith(".png"):
            print(f"Blurring {filename}...")
            source_path = os.path.join(source_dir, filename)
            output_filename = filename.replace("original", "blurred")
            output_path = os.path.join(blurred_dir, output_filename)

            img = Image.open(source_path)
            img = np.array(img).astype(np.float64) / 255.0
            img_gray = convert_RGB_RGBA_to_grayscale(img)
            m, n = img_gray.shape  # Assuming square images

            blurred_img = apply_motion_blur(img_gray, n, j_Aleft, k_Aleft, j_Aright, k_Aright)

            # Save the resulting image
            plt.imsave(output_path, blurred_img, cmap='gray')

# Function to recover the original image using LU decomposition
def recover_matrix_from_blurred_matrix_LU(blurred_matrix, n, j_Aleft, k_Aleft, j_Aright, k_Aright):
    """
    Recover the original image matrix from the blurred matrix using LU decomposition.

    Parameters:
    - blurred_matrix: np array, blurred image
    - n: int, size of the image (assuming square)
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix

    Returns:
    - X: np array, recovered image
    """
    # Construct Aleft and Aright matrices
    motion_blurring_Aleft = MotionTypeBlurringMatrix(n, k_Aleft, j_Aleft)
    Aleft = motion_blurring_Aleft.get_matrix()

    motion_blurring_Aright = MotionTypeBlurringMatrix(n, k_Aright, j_Aright)
    Aright = motion_blurring_Aright.get_matrix()

    # Invert Aright
    Aright_inv = np.linalg.inv(Aright)

    # Transform B
    B_transformed = blurred_matrix @ Aright_inv

    # Start CPU timer
    start_time = time.time()

    # Make Aleft invertible
    Aleft_reg = make_matrix_non_singular(Aleft, epsilon=1e-6)

    # Solve Aleft * X = B_transformed using LU decomposition
    lu, piv = lu_factor(Aleft_reg)
    X = lu_solve((lu, piv), B_transformed)

    # End CPU timer
    end_time = time.time()
    cpu_time = end_time - start_time

    # Clip values of X to be in the range [0, 1]
    X = np.clip(X, 0, 1)

    return X, cpu_time

# Function to recover all blurred files using LU
def recover_all_blurred_files_LU(blurred_dir, j_Aleft, k_Aleft, j_Aright, k_Aright, recovered_dir):
    """
    Recover all blurred PNG images using LU-based least squares.

    Parameters:
    - blurred_dir: str, path to blurred images
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix
    - recovered_dir: str, path to save recovered images
    """
    if not os.path.exists(recovered_dir):
        os.makedirs(recovered_dir)

    for filename in os.listdir(blurred_dir):
        if filename.lower().endswith(".png"):
            print(f"\nDeblurring {filename} using LU decomposition...")
            blurred_path = os.path.join(blurred_dir, filename)
            output_filename = filename.replace("blurred", "recovered_LU")
            output_path = os.path.join(recovered_dir, output_filename)

            # Load blurred image
            blurred_img = Image.open(blurred_path)
            blurred_img = np.array(blurred_img).astype(np.float64) / 255.0
            blurred_img_gray = convert_RGB_RGBA_to_grayscale(blurred_img)
            m, n = blurred_img_gray.shape  # Assuming square images

            # Recover the image and measure CPU time
            recovered_img, cpu_time = recover_matrix_from_blurred_matrix_LU(
                blurred_img_gray, n, j_Aleft, k_Aleft, j_Aright, k_Aright
            )
            print(f"Recovered image using LU in {cpu_time:.4f} seconds.")

            # Save the recovered image
            plt.imsave(output_path, recovered_img, cmap='gray')

            # Load the original image for error calculation
            original_filename = filename.replace("blurred", "original")
            original_path = os.path.join(source_image_dir, original_filename)
            if os.path.exists(original_path):
                original_img = Image.open(original_path)
                original_img = np.array(original_img).astype(np.float64) / 255.0
                original_img_gray = convert_RGB_RGBA_to_grayscale(original_img)

                # Compute relative forward error using Frobenius norm
                error_norm = np.linalg.norm(recovered_img - original_img_gray, 'fro')
                original_norm = np.linalg.norm(original_img_gray, 'fro')
                relative_error = error_norm / original_norm
                print(f"Relative Forward Error (LU): {relative_error:.6f}")
                PSNR = 10 * (np.log10((n**2)/((error_norm)**2)))
                print(f"the peak-signal-to-noise ratio: ", PSNR)
            else:
                print(f"Original image {original_filename} not found. Cannot compute error.")

# Function to recover the original image using QR decomposition
def recover_matrix_from_blurred_matrix_QR(blurred_matrix, n, j_Aleft, k_Aleft, j_Aright, k_Aright):
    """
    Recover the original image matrix from the blurred matrix by solving least square problems using QR decomposition and pseudoinversion.

    Parameters:
    - blurred_matrix: np array, blurred image
    - n: int, size of the image (assuming square)
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix

    Returns:
    - X: np array, recovered image
    - cpu_time: float, time taken in seconds
    """
    # Construct Aleft and Aright matrices
    motion_blurring_Aleft = MotionTypeBlurringMatrix(n, k_Aleft, j_Aleft)
    Aleft = motion_blurring_Aleft.get_matrix()

    motion_blurring_Aright = MotionTypeBlurringMatrix(n, k_Aright, j_Aright)
    Aright = motion_blurring_Aright.get_matrix()

    # Invert Aright
    Aright_inv = np.linalg.inv(Aright)

    # Transform the blurred image
    B = blurred_matrix @ Aright_inv

    # Start CPU timer
    start_time = time.time()

    # Perform QR decomposition on Aleft
    Qleft, Rleft = qr(Aleft, mode='economic')

    # Compute pseudoinverse of Rleft
    Rleft_pinv = np.linalg.pinv(Rleft)

    # Recover X using the formula: X = Rleft_pinv @ Qleft.T @ B
    X = Rleft_pinv @ Qleft.T @ B

    # End CPU timer
    end_time = time.time()
    cpu_time = end_time - start_time

    # Clip the values to [0, 1] to maintain valid image intensities
    X = np.clip(X, 0, 1)

    return X, cpu_time

# Function to recover all blurred files using QR
def recover_all_blurred_files_QR(blurred_dir, j_Aleft, k_Aleft, j_Aright, k_Aright, recovered_dir):
    """
    Recover all blurred PNG images.

    Parameters:
    - blurred_dir: str, path to blurred images
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix
    - recovered_dir: str, path to save recovered images
    """
    if not os.path.exists(recovered_dir):
        os.makedirs(recovered_dir)

    for filename in os.listdir(blurred_dir):
        if filename.lower().endswith(".png"):
            print(f"\nDeblurring {filename} using QR decomposition...")
            blurred_path = os.path.join(blurred_dir, filename)
            output_filename = filename.replace("blurred", "recovered_QR")
            output_path = os.path.join(recovered_dir, output_filename)

            # Load blurred image
            blurred_img = Image.open(blurred_path)
            blurred_img = np.array(blurred_img).astype(np.float64) / 255.0
            blurred_img_gray = convert_RGB_RGBA_to_grayscale(blurred_img)
            m, n = blurred_img_gray.shape  # Assuming square images

            # Recover the image and measure CPU time
            recovered_img, cpu_time = recover_matrix_from_blurred_matrix_QR(
                blurred_img_gray, n, j_Aleft, k_Aleft, j_Aright, k_Aright
            )
            print(f"Recovered image using QR in {cpu_time:.4f} seconds.")

            # Save the recovered image
            plt.imsave(output_path, recovered_img, cmap='gray')

            # Load the original image for error calculation
            original_filename = filename.replace("blurred", "original")
            original_path = os.path.join(source_image_dir, original_filename)
            if os.path.exists(original_path):
                original_img = Image.open(original_path)
                original_img = np.array(original_img).astype(np.float64) / 255.0
                original_img_gray = convert_RGB_RGBA_to_grayscale(original_img)

                # Compute relative forward error using Frobenius norm
                error_norm = np.linalg.norm(recovered_img - original_img_gray, 'fro')
                original_norm = np.linalg.norm(original_img_gray, 'fro')
                relative_error = error_norm / original_norm
                print(f"Relative Forward Error (QR): {relative_error:.6f}")
                PSNR = 10 * (np.log10((n**2)/((error_norm)**2)))
                print(f"the peak-signal-to-noise ratio: ", PSNR)
            else:
                print(f"Original image {original_filename} not found. Cannot compute error.")





def householder_qr(A):
    """
    Perform Householder QR decomposition with column permutations.
    
    Parameters:
    A (numpy.ndarray): An m x n matrix with m >= n.
    
    Returns:
    Q (numpy.ndarray): An m x m orthogonal matrix.
    R (numpy.ndarray): An m x n upper triangular matrix.
    """
    #  column-majored order
    A = np.asfortranarray(A, dtype=np.float64)
    
    m, n = A.shape
    R = A.copy()
    Q = np.eye(m, dtype=np.float64)
    
    
    for k in range(min(m-1, n)):
        # 提取列向量x
        x = R[k:, k]
        
        # 计算x的范数和alpha
        alpha = norm(x)
        if alpha == 0:
            continue
        
        # 构造Householder向量v
        v = x.copy()
        v[0] += alpha if x[0] >= 0 else -alpha
        v /= norm(v)
        
        # 仅更新R矩阵的下半部分
        R[k:, k:] -= 2.0 * np.outer(v, np.dot(v, R[k:, k:]))
        
        # 更新Q矩阵
        Q[:, k:] -= 2.0 * np.outer(Q[:, k:].dot(v), v)
    
    return Q, R







def householder_qr_pivoting(A, tol=1e-10):
    """
    Perform Householder QR decomposition with column pivoting.

    Parameters:
    - A (numpy.ndarray): An m x n matrix with m >= n.
    - tol (float): Tolerance for determining the rank of A.

    Returns:
    - P (numpy.ndarray): An n x n permutation matrix.
    - Q (numpy.ndarray): An m x m orthogonal matrix.
    - R1 (numpy.ndarray): An r x r upper triangular submatrix of R.
    - r (int): The rank of matrix A based on the tolerance.
    """
    # 确保A是连续的并使用float64类型
    A = np.asfortranarray(A, dtype=np.float64)
    m, n = A.shape

    # 初始化Q为单位矩阵
    Q = np.eye(m, dtype=np.float64)

    # 初始化置换数组
    perm = np.arange(n)

    r = n  # 初始化秩为n

    for k in range(min(m, n)):
        # 计算剩余列的欧几里得范数
        column_norms = norm(A[k:, k:], axis=0)

        # 找到范数最大的列索引
        pivot_col_relative = np.argmax(column_norms)
        pivot_col = pivot_col_relative + k
        max_norm = column_norms[pivot_col_relative]

        # 检查是否低于容差
        if max_norm < tol:
            r = k
            break

        # 记录置换
        if pivot_col != k:
            # 交换A的列
            A[:, [k, pivot_col]] = A[:, [pivot_col, k]]
            # 记录置换
            perm[[k, pivot_col]] = perm[[pivot_col, k]]

        # 提取x向量 extract v
        x = A[k:, k]
        v = x.copy()
        alpha = norm(x)
        v[0] += alpha if x[0] >= 0 else -alpha
        v /= norm(v)
        

        # 更新A的下半部分
        # A[k:, k:] = A[k:, k:] - 2 * np.outer(v, v.T @ A[k:, k:])
        # 使用更高效的运算顺序
        A[k:, k:] -= 2.0 * np.outer(v, v @ A[k:, k:])

        # 更新Q矩阵
        Q[:, k:] -= 2.0 * np.outer(Q[:, k:] @ v, v)

    # 构建置换矩阵P
    P = np.eye(n)[:, perm]

    # 提取R1
    R1 = A[:r, :r]

    return P, Q, R1, r

def recover_matrix_from_blurred_matrix_householderQR(blurred_matrix, n, j_Aleft, k_Aleft, j_Aright, k_Aright):
    """
    Recover the original image matrix from the blurred matrix by solving least square problems using my own householder QR decomposition and pseudoinversion.

    Parameters:
    - blurred_matrix: np array, blurred image
    - n: int, size of the image (assuming square)
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix

    Returns:
    - X: np array, recovered image
    - cpu_time: float, time taken in seconds
    """
    # Construct Aleft and Aright matrices
    motion_blurring_Aleft = MotionTypeBlurringMatrix(n, k_Aleft, j_Aleft)
    Aleft = motion_blurring_Aleft.get_matrix()

    motion_blurring_Aright = MotionTypeBlurringMatrix(n, k_Aright, j_Aright)
    Aright = motion_blurring_Aright.get_matrix()

    # Invert Aright
    Aright_inv = np.linalg.inv(Aright)

    # Transform the blurred image
    B = blurred_matrix @ Aright_inv

    # Start CPU timer
    start_time = time.time()

    # Perform QR decomposition on Aleft
    Qleft, Rleft = householder_qr(Aleft)
    # Compute pseudoinverse of Rleft
    Rleft_pinv = np.linalg.pinv(Rleft)

    # Recover X using the formula: X = Rleft_pinv @ Qleft.T @ B
    X = Rleft_pinv @ Qleft.T @ B

    # End CPU timer
    end_time = time.time()
    cpu_time = end_time - start_time

    # Clip the values to [0, 1] to maintain valid image intensities
    X = np.clip(X, 0, 1)

    return X, cpu_time

def recover_all_blurred_files_householderQR(blurred_dir, j_Aleft, k_Aleft, j_Aright, k_Aright, recovered_dir):
    """
    Recover all blurred PNG images.

    Parameters:
    - blurred_dir: str, path to blurred images
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix
    - recovered_dir: str, path to save recovered images
    """
    if not os.path.exists(recovered_dir):
        os.makedirs(recovered_dir)

    for filename in os.listdir(blurred_dir):
        if filename.lower().endswith(".png"):
            print(f"\nDeblurring {filename} using my own householder QR decomposition...")
            blurred_path = os.path.join(blurred_dir, filename)
            output_filename = filename.replace("blurred", "recovered_householderQR")
            output_path = os.path.join(recovered_dir, output_filename)

            # Load blurred image
            blurred_img = Image.open(blurred_path)
            blurred_img = np.array(blurred_img).astype(np.float64) / 255.0
            blurred_img_gray = convert_RGB_RGBA_to_grayscale(blurred_img)
            m, n = blurred_img_gray.shape  # Assuming square images

            # Recover the image and measure CPU time
            recovered_img, cpu_time = recover_matrix_from_blurred_matrix_householderQR(
                blurred_img_gray, n, j_Aleft, k_Aleft, j_Aright, k_Aright
            )
            print(f"Recovered image using my own householderQR in {cpu_time:.4f} seconds.")

            # Save the recovered image
            plt.imsave(output_path, recovered_img, cmap='gray')

            # Load the original image for error calculation
            original_filename = filename.replace("blurred", "original")
            original_path = os.path.join(source_image_dir, original_filename)
            if os.path.exists(original_path):
                original_img = Image.open(original_path)
                original_img = np.array(original_img).astype(np.float64) / 255.0
                original_img_gray = convert_RGB_RGBA_to_grayscale(original_img)

                # Compute relative forward error using Frobenius norm
                error_norm = np.linalg.norm(recovered_img - original_img_gray, 'fro')
                original_norm = np.linalg.norm(original_img_gray, 'fro')
                relative_error = error_norm / original_norm
                print(f"Relative Forward Error (QR): {relative_error:.6f}")
                PSNR = 10 * (np.log10((n**2)/((error_norm)**2)))
                print(f"the peak-signal-to-noise ratio: ", PSNR)
            else:
                print(f"Original image {original_filename} not found. Cannot compute error.")

def recover_matrix_from_blurred_matrix_householder_pivoting_QR(blurred_matrix, n, j_Aleft, k_Aleft, j_Aright, k_Aright):
    """
    Recover the original image matrix from the blurred matrix by solving least square problem using QR decomposition with pivoting

    Parameters:
    - blurred_matrix: np array, blurred image
    - n: int, size of the image (assuming square)
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix

    Returns:
    - X: np array, recovered image
    - cpu_time: float, time taken in seconds
    """

    # memory optimization
    blurred_matrix = np.ascontiguousarray(blurred_matrix, dtype=np.float64)
    
    # Construct Aleft and Aright matrices
    motion_blurring_Aleft = MotionTypeBlurringMatrix(n, k_Aleft, j_Aleft)
    Aleft = motion_blurring_Aleft.get_matrix()
    Aleft = np.asfortranarray(Aleft, dtype=np.float64)  # column majored order to power blas


    motion_blurring_Aright = MotionTypeBlurringMatrix(n, k_Aright, j_Aright)
    Aright = motion_blurring_Aright.get_matrix()
    Aright = np.asfortranarray(Aright, dtype=np.float64)  


    # Invert Aright
    Aright_inv = np.linalg.inv(Aright)

    # Transform the blurred image
    B = blurred_matrix @ Aright_inv

    # Start CPU timer
    start_time = time.time()

    # Perform QR decomposition with pivoting on Aleft
    P, Q, R1, r = householder_qr_pivoting(Aleft, tol=1e-10)
    
    # Compute Q^T * B
    Qt_B = Q.T @ B
    
    # Extract the first r rows of Q^T * B
    C1 = Qt_B[:r, :]
    
    # Solve R1 * Y = C1 for Y
    Y = solve_triangular(R1, C1, lower=False)
    
    # Initialize X_temp with zeros (n x n)
    n = Aleft.shape[1]
    X_temp = np.zeros((n, n),dtype=np.float64)
    
    # Populate the first r rows with Y
    X_temp[:r, :] = Y
    
    # followed with the solution of least square problem using QR in lecture notes 14
    X = P @ X_temp

    # End CPU timer
    end_time = time.time()
    cpu_time = end_time - start_time

    # Clip the values to [0, 1] to maintain valid image intensities
    X = np.clip(X, 0, 1)

    return X, cpu_time

def recover_all_blurred_files_householder_pivoting_QR(blurred_dir, j_Aleft, k_Aleft, j_Aright, k_Aright, recovered_dir):
    """
    Recover all blurred PNG images.

    Parameters:
    - blurred_dir: str, path to blurred images
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix
    - recovered_dir: str, path to save recovered images
    """
    if not os.path.exists(recovered_dir):
        os.makedirs(recovered_dir)

    for filename in os.listdir(blurred_dir):
        if filename.lower().endswith(".png"):
            print(f"\nDeblurring {filename} using my own householder with pivoting QR decomposition...")
            blurred_path = os.path.join(blurred_dir, filename)
            output_filename = filename.replace("blurred", "recovered_householder_pivoting_QR")
            output_path = os.path.join(recovered_dir, output_filename)

            # Load blurred image
            blurred_img = Image.open(blurred_path)
            blurred_img = np.array(blurred_img).astype(np.float64) / 255.0
            blurred_img_gray = convert_RGB_RGBA_to_grayscale(blurred_img)
            m, n = blurred_img_gray.shape  # Assuming square images

            # Recover the image and measure CPU time
            recovered_img, cpu_time = recover_matrix_from_blurred_matrix_householder_pivoting_QR(
                blurred_img_gray, n, j_Aleft, k_Aleft, j_Aright, k_Aright
            )
            print(f"Recovered image using my own householder pivoting QR in {cpu_time:.4f} seconds.")

            # Save the recovered image
            plt.imsave(output_path, recovered_img, cmap='gray')

            # Load the original image for error calculation
            original_filename = filename.replace("blurred", "original")
            original_path = os.path.join(source_image_dir, original_filename)
            if os.path.exists(original_path):
                original_img = Image.open(original_path)
                original_img = np.array(original_img).astype(np.float64) / 255.0
                original_img_gray = convert_RGB_RGBA_to_grayscale(original_img)

                # Compute relative forward error using Frobenius norm and PSNR
                error_norm = np.linalg.norm(recovered_img - original_img_gray, 'fro')
                original_norm = np.linalg.norm(original_img_gray, 'fro')
                relative_error = error_norm / original_norm
                print(f"Relative Forward Error (QR): {relative_error:.6f}")
                PSNR = 10 * (np.log10((n**2)/((error_norm)**2)))
                print(f"the peak-signal-to-noise ratio: ", PSNR)
            else:
                print(f"Original image {original_filename} not found. Cannot compute error.")


def recover_matrix_from_blurred_matrix_scipy_pivoting_QR(blurred_matrix, n, j_Aleft, k_Aleft, j_Aright, k_Aright):
    """
    Recover the original image matrix from the blurred matrix by solving least square problem using QR decomposition with pivoting

    Parameters:
    - blurred_matrix: np array, blurred image
    - n: int, size of the image (assuming square)
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix

    Returns:
    - X: np array, recovered image
    - cpu_time: float, time taken in seconds
    """

    blurred_matrix = np.ascontiguousarray(blurred_matrix, dtype=np.float64)
    
    # Construct Aleft and Aright matrices
    motion_blurring_Aleft = MotionTypeBlurringMatrix(n, k_Aleft, j_Aleft)
    Aleft = motion_blurring_Aleft.get_matrix()
    Aleft = np.asfortranarray(Aleft, dtype=np.float64)  # Column majored order


    motion_blurring_Aright = MotionTypeBlurringMatrix(n, k_Aright, j_Aright)
    Aright = motion_blurring_Aright.get_matrix()
    Aright = np.asfortranarray(Aright, dtype=np.float64)  


    # Invert Aright
    Aright_inv = np.linalg.inv(Aright)

    # Transform the blurred image
    B = blurred_matrix @ Aright_inv

    # Start CPU timer
    start_time = time.time()

    Q, R, p = qr(blurred_matrix, pivoting=True, mode='full')
    r = np.linalg.matrix_rank(R)
    R1 = R[:r, :r]

    # Compute Q^T * B
    Qt_B = Q.T @ B
    
    # Extract the first r rows of Q^T * B
    C1 = Qt_B[:r, :]
    
    # Solve R1 * Y = C1 for Y
    Y = solve_triangular(R1, C1, lower=False)
    
    # Initialize X_temp with zeros (n x n)
    n = Aleft.shape[1]
    X_temp = np.zeros((n, n),dtype=np.float64)
    
    # Populate the first r rows with Y
    X_temp[:r, :] = Y

    # followed with the solution of least square problem using QR in lecture notes 14
    X = X_temp[:, p]

    # End CPU timer
    end_time = time.time()
    cpu_time = end_time - start_time

    # Clip the values to [0, 1] to maintain valid image intensities
    X = np.clip(X, 0, 1)

    return X, cpu_time

def recover_all_blurred_files_scipy_pivoting_QR(blurred_dir, j_Aleft, k_Aleft, j_Aright, k_Aright, recovered_dir):
    """
    Recover all blurred PNG images.

    Parameters:
    - blurred_dir: str, path to blurred images
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix
    - recovered_dir: str, path to save recovered images
    """
    if not os.path.exists(recovered_dir):
        os.makedirs(recovered_dir)

    for filename in os.listdir(blurred_dir):
        if filename.lower().endswith(".png"):
            print(f"\nDeblurring {filename} using scipy householder with pivoting QR decomposition...")
            blurred_path = os.path.join(blurred_dir, filename)
            output_filename = filename.replace("blurred", "recovered_scipy_pivoting_QR")
            output_path = os.path.join(recovered_dir, output_filename)

            # Load blurred image
            blurred_img = Image.open(blurred_path)
            blurred_img = np.array(blurred_img).astype(np.float64) / 255.0
            blurred_img_gray = convert_RGB_RGBA_to_grayscale(blurred_img)
            m, n = blurred_img_gray.shape  # Assuming square images

            # Recover the image and measure CPU time
            recovered_img, cpu_time = recover_matrix_from_blurred_matrix_scipy_pivoting_QR(
                blurred_img_gray, n, j_Aleft, k_Aleft, j_Aright, k_Aright
            )
            print(f"Recovered image using scipy pivoting QR in {cpu_time:.4f} seconds.")

            # Save the recovered image
            plt.imsave(output_path, recovered_img, cmap='gray')

            # Load the original image for error calculation
            original_filename = filename.replace("blurred", "original")
            original_path = os.path.join(source_image_dir, original_filename)
            if os.path.exists(original_path):
                original_img = Image.open(original_path)
                original_img = np.array(original_img).astype(np.float64) / 255.0
                original_img_gray = convert_RGB_RGBA_to_grayscale(original_img)

                # Compute relative forward error using Frobenius norm and PSNR
                error_norm = np.linalg.norm(recovered_img - original_img_gray, 'fro')
                original_norm = np.linalg.norm(original_img_gray, 'fro')
                relative_error = error_norm / original_norm
                print(f"Relative Forward Error (QR): {relative_error:.6f}")
                PSNR = 10 * (np.log10((n**2)/((error_norm)**2)))
                print(f"the peak-signal-to-noise ratio: ", PSNR)
            else:
                print(f"Original image {original_filename} not found. Cannot compute error.")

def recover_matrix_from_blurred_matrix_QR_convertRnonsingular(blurred_matrix, n, j_Aleft, k_Aleft, j_Aright, k_Aright):
    """
    Recover the original image matrix from the blurred matrix by solving least square problems using QR decomposition and pseudoinversion.

    Parameters:
    - blurred_matrix: np array, blurred image
    - n: int, size of the image (assuming square)
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix

    Returns:
    - X: np array, recovered image
    - cpu_time: float, time taken in seconds
    """
    # Construct Aleft and Aright matrices
    motion_blurring_Aleft = MotionTypeBlurringMatrix(n, k_Aleft, j_Aleft)
    Aleft = motion_blurring_Aleft.get_matrix()

    motion_blurring_Aright = MotionTypeBlurringMatrix(n, k_Aright, j_Aright)
    Aright = motion_blurring_Aright.get_matrix()

    # Invert Aright
    Aright_inv = np.linalg.inv(Aright)

    # Transform the blurred image
    B = blurred_matrix @ Aright_inv

    # Start CPU timer
    start_time = time.time()

    # Perform QR decomposition on Aleft
    Qleft, Rleft = qr(Aleft, mode='economic')

    Rleft=make_matrix_non_singular(Rleft,epsilon=1e-6)
    # Compute pseudoinverse of Rleft
    Rleft_inv = np.linalg.inv(Rleft)

    # Recover X using the formula: X = Rleft_pinv @ Qleft.T @ B
    X = Rleft_inv @ Qleft.T @ B

    # End CPU timer
    end_time = time.time()
    cpu_time = end_time - start_time

    # Clip the values to [0, 1] to maintain valid image intensities
    X = np.clip(X, 0, 1)

    return X, cpu_time

def recover_all_blurred_files_QR_withRnonsingular(blurred_dir, j_Aleft, k_Aleft, j_Aright, k_Aright, recovered_dir):
    """
    Recover all blurred PNG images.

    Parameters:
    - blurred_dir: str, path to blurred images
    - j_Aleft, k_Aleft: Parameters for Aleft matrix
    - j_Aright, k_Aright: Parameters for Aright matrix
    - recovered_dir: str, path to save recovered images
    """
    if not os.path.exists(recovered_dir):
        os.makedirs(recovered_dir)

    for filename in os.listdir(blurred_dir):
        if filename.lower().endswith(".png"):
            print(f"\nDeblurring {filename} using QR decomposition with R converted nonsingular...")
            blurred_path = os.path.join(blurred_dir, filename)
            output_filename = filename.replace("blurred", "recovered_QR_withRnonsingular")
            output_path = os.path.join(recovered_dir, output_filename)

            # Load blurred image
            blurred_img = Image.open(blurred_path)
            blurred_img = np.array(blurred_img).astype(np.float64) / 255.0
            blurred_img_gray = convert_RGB_RGBA_to_grayscale(blurred_img)
            m, n = blurred_img_gray.shape  # Assuming square images

            # Recover the image and measure CPU time
            recovered_img, cpu_time = recover_matrix_from_blurred_matrix_QR_convertRnonsingular(
                blurred_img_gray, n, j_Aleft, k_Aleft, j_Aright, k_Aright
            )
            print(f"Recovered image using QR with R converted nonsingular in {cpu_time:.4f} seconds.")

            # Save the recovered image
            plt.imsave(output_path, recovered_img, cmap='gray')

            # Load the original image for error calculation
            original_filename = filename.replace("blurred", "original")
            original_path = os.path.join(source_image_dir, original_filename)
            if os.path.exists(original_path):
                original_img = Image.open(original_path)
                original_img = np.array(original_img).astype(np.float64) / 255.0
                original_img_gray = convert_RGB_RGBA_to_grayscale(original_img)

                # Compute relative forward error using Frobenius norm
                error_norm = np.linalg.norm(recovered_img - original_img_gray, 'fro')
                original_norm = np.linalg.norm(original_img_gray, 'fro')
                relative_error = error_norm / original_norm
                print(f"Relative Forward Error (QR): {relative_error:.6f}")
                PSNR = 10 * (np.log10((n**2)/((error_norm)**2)))
                print(f"the peak-signal-to-noise ratio: ", PSNR)
            else:
                print(f"Original image {original_filename} not found. Cannot compute error.")





# Main execution block
def main():
    # # Step 1: Apply motion blur to all original images
    # print("Starting blurring process...\n")
    # blur_all_png_files(
    #     source_dir=source_image_dir,
    #     j_Aleft=j_Aleft,
    #     k_Aleft=k_Aleft,
    #     j_Aright=j_Aright,
    #     k_Aright=k_Aright,
    #     blurred_dir=blurred_image_dir
    # )
    # print("\nBlurring process completed.\n")

    # # Step 2: Recover blurred images using LU decomposition
    # print("\nStarting recovery using LU decomposition...")
    # recover_all_blurred_files_LU(
    #     blurred_dir=blurred_image_dir,
    #     j_Aleft=j_Aleft,
    #     k_Aleft=k_Aleft,
    #     j_Aright=j_Aright,
    #     k_Aright=k_Aright,
    #     recovered_dir=recovered_image_dir_LU,
    # )
    # print("\nRecovery using LU decomposition completed.\n\n")

    # # Step 3: Recover blurred images using QR decomposition
    # print("\nStarting recovery using QR decomposition...")
    # recover_all_blurred_files_QR(
    #     blurred_dir=blurred_image_dir,
    #     j_Aleft=j_Aleft,
    #     k_Aleft=k_Aleft,
    #     j_Aright=j_Aright,
    #     k_Aright=k_Aright,
    #     recovered_dir=recovered_image_dir_QR,
    # )
    # print("\nRecovery using QR decomposition completed.\n\n")

    # # Step 4: Recover blurred images using my own householder QR decomposition
    # print("\nStarting recovery using my own householder QR decomposition...")
    # recover_all_blurred_files_householderQR(
    #     blurred_dir=blurred_image_dir,
    #     j_Aleft=j_Aleft,
    #     k_Aleft=k_Aleft,
    #     j_Aright=j_Aright,
    #     k_Aright=k_Aright,
    #     recovered_dir=recovered_image_dir_householderQR,
    # )
    # print("\nRecovery using my own householder QR decomposition completed.\n\n")

    # # Step 5: Recover blurred images using my own householder QR pivoting decomposition
    # print("\nStarting recovery using my own householder pivoting QR decomposition...")
    # recover_all_blurred_files_householder_pivoting_QR(
    #     blurred_dir=blurred_image_dir,
    #     j_Aleft=j_Aleft,
    #     k_Aleft=k_Aleft,
    #     j_Aright=j_Aright,
    #     k_Aright=k_Aright,
    #     recovered_dir=recovered_image_dir_householder_pivoting_QR,
    # )
    # print("\nRecovery using my own householder pivoting QR decomposition completed.\n\n")

    # # Step 6: Recover blurred images using scipy QR pivoting decomposition
    # print("\nStarting recovery using scipy pivoting QR decomposition...")
    # recover_all_blurred_files_householder_pivoting_QR(
    #     blurred_dir=blurred_image_dir,
    #     j_Aleft=j_Aleft,
    #     k_Aleft=k_Aleft,
    #     j_Aright=j_Aright,
    #     k_Aright=k_Aright,
    #     recovered_dir=recovered_image_dir_scipy_pivoting_QR,
    # )
    # print("\nRecovery using scipy pivoting QR decomposition completed.\n\n")

     # Step 7: Recover blurred images using  QR pivoting decomposition with R converted non-singular
    print("\nStarting recovery using  QR decomposition with R converted nonsingular..")
    recover_all_blurred_files_QR_withRnonsingular(
        blurred_dir=blurred_image_dir,
        j_Aleft=j_Aleft,
        k_Aleft=k_Aleft,
        j_Aright=j_Aright,
        k_Aright=k_Aright,
        recovered_dir=recovered_image_dir_QR_withRnonsingular,
    )
    print("\nRecovery using scipy pivoting QR decomposition with R converted nonsingular completed.\n\n")


    

if __name__ == "__main__":
    main()




































